module.exports = function (bot, args, channel, inDashboard) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (index !== -1) {
      bot.info.settings.IGNORED_PLAYERS.splice(index, 1);
  ...
  if (validArgs.length) if (!inDashboard) channel.send(`\`${validArgs.join(', ')}\` removed from the Ignored Players list.`);
  if (invalidArgs.length) if (!inDashboard) channel.send(`\`${invalidArgs.join(', ')}\` not found in the Ignored Players list.`);
