...
module.exports = async function (bot, args, channel, inDashboard) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (result === 'No clan') {
      ...
      await page.click('div[id^="radioRowundefined4"] > div:first-child > div:first-child');
      ...
      } else if (resultMsg.includes('is our new Ally')) {
  ...
  if (validArgs.length) channel.send(`\`${validArgs.join(', ')}\`: have been allied!`);
  ...
  if (unableToArgs.length) channel.send(`\`${unableToArgs.join(', ')}\`: unable to be allied. Try again later.`);
  if (alreadyArgs.length) channel.send(`\`${alreadyArgs.join(', ')}\`: already allies!`);
};