module.exports = function (bot, args, channel, inDashboard) {
  ...
  for (let i = 0; i < args.length; i++) {
    if (lcSettingsArr.indexOf(args[i].toLowerCase()) === -1) {
      bot.info.settings.IGNORED_CLANS.push(args[i]);
  ...
  if (validArgs.length) if (!inDashboard) channel.send(`\`${validArgs.join(', ')}\` added to the Ignored Clans list.`);
  if (invalidArgs.length) if (!inDashboard) channel.send(`\`${invalidArgs.join(', ')}\` already in the Ignored Clans list.`);
